...
namespace EmailClientATM
{
    public partial class ImportantForm : Form
    {
        ...
        public ImportantForm(string email)
        {
            InitializeComponent();
            emailUser = email;
            PopulateImportantForm();
        }
        private void PopulateImportantForm()
        {
            var nw = ConfigurationManager.ConnectionStrings["nw"];
            using (var con = new SqlConnection(nw.ConnectionString))
            {
                con.Open();
                var cmd = new SqlCommand("GetImportanteByEmail", con);
